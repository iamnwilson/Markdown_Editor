...
def convert_extras(extras):
    ...
    return Requirement(f"placeholder{extras.lower()}").extras if extras else set()
...
def parse_editable(editable_req):
    ...
    if os.path.isdir(url_no_extras):
        ...
        if not os.path.exists(setup_py) and not os.path.exists(setup_cfg):
            msg = f'File "setup.py" or "setup.cfg" not found. Directory cannot be installed in editable mode: {os.path.abspath(url_no_extras)}'
...
def parse_req_from_line(name, line_source):
    ...
    def with_source(text):
        # type: (str) -> str
        return f'{text} (from {line_source})' if line_source else text
