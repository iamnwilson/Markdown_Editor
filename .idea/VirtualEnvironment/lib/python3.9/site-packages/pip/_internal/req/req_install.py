...
class InstallRequirement:
    ...
    def name(self):
        # type: () -> Optional[str]
        return None if self.req is None else pkg_resources.safe_name(self.req.name)
        ...
        if version_compatible := (
            ...
            if (
                ...
                raise InstallationError(
                    f"Will not install to the user site because it will lack sys.path precedence to {existing_dist.project_name} in {existing_dist.location}"
                )
    ...
    def install(
        ...
        if success and self.legacy_install_reason == 8368:
            deprecated(
                reason=f"{self.name} was installed using the legacy 'setup.py install' method, because a wheel could not be built for it.",
